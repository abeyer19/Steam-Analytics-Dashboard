...
achievements_data = pd.read_csv('Scrapers/steam_data_achievements/achievements.csv')
...
# 7. Achievements Counts
achieve_cols = ['appid', 'achievement_apiname']
achievements_data = achievements_data[achieve_cols]
achievements_data = achievements_data.rename(columns={'appid': 'steam_appid'})
achievements_data = achievements_data.groupby('steam_appid').size().reset_index(name='achievements_count')
...
revenue_cols = ['steam_appid', 'initial_price']
revenue_data = revenue_data[revenue_cols]


def limit_and_pivot(df: pd.DataFrame, col_name, top_n = None) -> pd.DataFrame:
...
genres_final = limit_and_pivot(genres_data, 'genres_description', 10)
tags_final = limit_and_pivot(tags_data, 'tag_name', 50)
cats_final = limit_and_pivot(cats_data, 'cats_description', 25)
...
master_data = details_data.merge(reviews_data, how='left', on='steam_appid') \
    ...
    .merge(achievements_data, how='left', on='steam_appid') \
